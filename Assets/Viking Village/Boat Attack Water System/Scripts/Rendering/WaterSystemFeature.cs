...
namespace WaterSystem
{
    public class WaterSystemFeature : ScriptableRendererFeature
    {
        ...
        class WaterFxPass : ScriptableRenderPass
        {
            ...
            public WaterFxPass()
            {
                // RTHandle allocation is done in Configure where we have the camera descriptor.
            ...
            public override void Configure(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
            {
                ...
                // // allocate an RTHandle for the lifetime of this camera render (RTHandles are preferred over RenderTargetHandle)
                // if (m_WaterFX == null)
                // {
                //     m_WaterFX = RTHandles.Alloc(cameraTextureDescriptor, filterMode: FilterMode.Bilinear, name: "_WaterFXMap");
                // }
                ...
                // allocate RTHandle (use RTHandle API, not TemporaryRT)
                if (m_WaterFX == null)
                    m_WaterFX = RTHandles.Alloc(cameraTextureDescriptor, filterMode: FilterMode.Bilinear, name: "_WaterFXMap");

                // set as target and clear
                ConfigureTarget(m_WaterFX);
                ConfigureClear(ClearFlag.Color, m_ClearColor);
            ...
            public override void OnCameraCleanup(CommandBuffer cmd)
            {
                ...
                // cmd.ReleaseTemporaryRT(m_WaterFX.nameID);
                // // release RTHandle when camera render is done
                // if (m_WaterFX != null)
                // {
